...
echo "$($PSQL "select format('%1s|%2s', games.year, teams.name) from games inner join teams on games.winner_id = teams.team_id where games.round='Final' order by games.year")"
